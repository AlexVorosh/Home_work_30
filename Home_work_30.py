...
def test_all_pets_cards(driver):
   #Вводим email
    driver.find_element(By.ID, 'email').send_keys('voroshilovs@yandex.ru')
   #Вводим пароль
    driver.find_element(By.ID, 'pass').send_keys('EEdkLl992Dddf')
   ...
   # Проверяем, что мы оказались на главной странице пользователя
    assert driver.find_element(By.TAG_NAME, 'H2').text == "alexvor"

   # Объявили три переменные, в которых записали все найденные элементы на странице: в images — картинки питомцев, в names — имена, в ages — возраст.
...
def test_pets_table(driver):
    ...
    WebDriverWait(driver, 10).until(EC.visibility_of_element_located((By.XPATH, '//a[text()=\"Мои питомцы\"]')))
